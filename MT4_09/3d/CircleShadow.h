...
public: // メンバ関数
	...
	inline void SetCasterPos(const Vec3& casterPos) { this->casterPos = casterPos; }

	/// <summary>
	/// キャスター座標を取得
	/// </summary>
	/// <returns>キャスター座標</returns>
	inline const Vec3& GetCasterPos() { return casterPos; }
	...
	inline void SetAtten(const Vec3& atten) { this->atten = atten; }

	/// <summary>
	/// 距離減衰係数を取得
	/// </summary>
	/// <returns>ライト距離減衰係数</returns>
	inline const Vec3& GetAtten() { return atten; }
	...
	inline const Vec2& GetFactorAngleCos() { return factorAngleCos; }
...
private: // メンバ変数
	...
	// キャスター座標（ワールド座標系）
	Vec3 casterPos = { 0,0,0 };
	// 距離減衰係数
	Vec3 atten = { 0.5f, 0.6f, 0.0f };
	// 減衰角度
	Vec2 factorAngleCos = { 0.2f, 0.5f };
	// 有効フラグ
	bool active = false;
};
